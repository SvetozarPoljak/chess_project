...
{
    m_moduleAddress = QBluetoothAddress("94:E3:6D:XX:XX:XX");
...
{
    // Create controller for BLE device via its fixed MAC address
    m_controller = QLowEnergyController::createCentral(m_moduleAddress, this);
    ...
    connect(m_controller, &QLowEnergyController::discoveryFinished, this, [this]() {
        ...
        if (m_controller->services().contains(SERIAL_SERVICE_UUID)) {
            m_service = m_controller->createService(SERIAL_SERVICE_UUID, this);
    ...
    // Starting asinhronous connection
